...
void reduce(vector<int> &h, int &x, int maxsum)
{
    if(h[x]==maxsum)
        return;
...
{
    fillsum(h1);fillsum(h2);fillsum(h3);
    int maxsum,x=h1.size()-1,y=h2.size()-1,z=h3.size()-1;
    
    while(true)
    {
        maxsum=min(h1[x],min(h2[y],h3[z]));
